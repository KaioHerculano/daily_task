...
class StudyDomainModelsTest(TestCase):
    ...
    def test_session_time_properties_with_pauses(self):
        ...
        SessionPause.objects.create(
            session=session,
            pause_start=start_time + timedelta(minutes=90),
            pause_end=start_time + timedelta(minutes=95),
        )

        self.assertEqual(session.gross_time.total_seconds(), 7200)
        self.assertEqual(
            session.paused_time.total_seconds(), 1200
        )  # 20 minutes = 1200 seconds
        self.assertEqual(
            session.net_time.total_seconds(), 6000
    ...
    def test_gross_time_when_session_in_progress(self):
        ...
        # gross_time uses timezone.now() if end_time is None
        # It should be roughly 1800 seconds
        gross = session.gross_time.total_seconds()
        self.assertGreaterEqual(gross, 1800)
        self.assertLess(gross, 1805)  # 5 seconds leeway for test execution time
