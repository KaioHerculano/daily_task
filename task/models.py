...
class TaskDayManager(models.Manager):
    ...
    def get_monthly_study_counts(self, user, start_year, end_year):
        return self.filter(user=user, date__year__gte=start_year, date__year__lte=end_year).annotate(month=TruncMonth('date')).values('month').annotate(total=Count('id')).order_by('month')
...
class StudySession(models.Model):
    ...
    objective_achieved = models.CharField(max_length=10, choices=ObjectiveAchieved.choices, default=ObjectiveAchieved.PENDING)
    ...
    focus_score = models.IntegerField(validators=[MinValueValidator(1), MaxValueValidator(5)], null=True, blank=True)
...
class SessionPause(models.Model):
    session = models.ForeignKey(StudySession, on_delete=models.CASCADE, related_name='pauses')
