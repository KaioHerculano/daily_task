...
def send_daily_reminders():
    ...
    users_studied_today = TaskDay.objects.filter(date=today).values_list(
        "user_id", flat=True
    )

    users_reminded_today = DailyReminderLog.objects.filter(date=today).values_list(
        "user_id", flat=True
    )

    target_users = (
        User.objects.filter(is_active=True)
        .exclude(id__in=users_studied_today)
        .exclude(id__in=users_reminded_today)
    )
    ...
    logger.info(
        f"[LEMBRETE] Lembretes processados para {count} usuários na data {today}."
    )
    return count
